...
{% for school in EDUCATION %}
    \item 
    \splitrow{\textbf{ {{ school.name }} }}{\textbf{ {{ school.location }} }}\\
    \splitrow{\emph{ {{ school.degree }} }}{\emph{ {{ school.dates }} }}
{% endfor %}          
\end{itemize}

\hrule
\vspace{-0.4em}
\subsection*{\so{VOLUNTEERING}}
...
{% for activity in VOLUNTEERING %}
    \item
      \splitrow{\textbf{ {{ activity.name }} }}{\textbf{ {{ activity.location }} }}\\
    {% for position in activity.positions %}\splitrow
        {\emph{ {{ position.title }} }}
        {\emph{ {{ position.dates }} }}
    {% endfor %}
